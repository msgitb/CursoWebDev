...
app.post("/jokes", (req, res) => {
  ...
  if (
    ...
    const jokeToPush = { id: jokes.length + 1, jokeText: text, jokeType: type };
    jokes.push(jokeToPush);
    res.status(200).send(jokeToPush);
  } else {
    res.status(400).send("Bad Request");
  }
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  if (
    ...
    var jokeToPut = jokes.filter((joke) => {
      return joke.id == id;
    })[0];
...
jokes.forEach((joke) => {
  if (!jkTypes.includes(joke.jokeType)) jkTypes.push(joke.jokeType);
});
