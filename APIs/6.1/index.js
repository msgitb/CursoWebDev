...
app.patch("/api/posts/:id", (req, res) => {
  ...
  if (id < 0 || id > posts.length) {
    ...
    var postToPatch = posts.find((post) => post.id === id);
    const index = posts.findIndex((post) => post === postToPatch);
    postToPatch = {
      id: id,
      title: title || postToPatch.title,
      content: content || postToPatch.content,
      author: author || postToPatch.author,
      date: new Date().toUTCString(),
    };
    posts[index] = postToPatch;
